...
class MemoryManager {
  constructor() {
    _defineProperty(this, "empty_stats", {
      total_active_time: 0,
    ...
    _defineProperty(this, "empty_tab", {
      tabId: null,
      url: null,
      full_url: null,
      statistics: {},
  ...
  async restoreTab(tabId) {
    ...
    let restoredTab = this.closed_history.filter(tab => {
      return tab.tabId === tabId;
    })[0]; // let cache = LRUfactory.fromJSON(restoredTab.cache);

    let tab = await new Promise((resolve, reject) => {
      chrome.tabs.create({
        url: restoredTab.full_url,
        active: false
      }, tab => {
    ...
    await this.createTab(tab);
    this.tabs[tab.id].statistics = copy(restoredTab.statistics); // this.tabs[tab.tabId].cache = cache;  // do not restore cache as history is lost
