class PolicyManager {
    ...
    static cleanRuntimeWindows(windows) {
        ...
        for (var i = 0; i < unknown_windows.length; i++) {
            delete memoryManager.runtime_events.last_policy_runs[
                unknown_windows[i]
            ]
        }
    }
        ...
        if (tabs.length > memoryManager.settings.policy.target_tabs) {
            ...
            if (this.exponentialTrigger(tabs, windowId)) {
                ...
                let scores = await Promise.all(
                    _.map(tabs, (tab) => Scorer.score(tab))
                )
                ...
                logger(
                    windowId
                        .toString()
                        .concat(' window scored: ', JSON.stringify(scores))
                )
                ...
                if (
                    count <
                    tabs.length - memoryManager.settings.policy.target_tabs
                ) {
                    await this.killTab(
                        deleteMe,
        ...
        return (
            ...
            memoryManager.settings.policy.min_time *
                Math.pow(
                    ...
                    Math.max(
                        0,
                        n_tabs - memoryManager.settings.policy.target_tabs
                    )
                )
        )
    }
