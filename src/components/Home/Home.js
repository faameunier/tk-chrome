    ...
    switch (listToBeRendered) {
      ...
      default:
        ...
        selectedList = selectedList.map((website, i) => {
          ...
          const formatted_deletion_time = deletionTime.toTimeString().
              split(' ')[0];
          website.hours_deletion = formatted_deletion_time.split(
              ':',
          )[0];
          website.minutes_deletion = formatted_deletion_time.split(
              ':',
          )[1];
          if (
              website.title &&
              website.title.length > MAX_LENGTH_TITLE
          ) {
            website.title = website.title.substring(0, MAX_LENGTH_TITLE).
                concat('...');
          }
          return website;
        });
